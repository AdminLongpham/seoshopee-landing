...
| **Domain (SEO)** | Thay `acchoanglong.com` → domain thật ở **4 nơi**: `index.html` (`og:url`, `canonical`, JSON-LD), `sitemap.xml`, `robots.txt`. Mẹo: tìm chữ `acchoanglong.com` để thay nhanh. |
...
- **Hosting/cPanel** (vd của acchoanglong.com): upload toàn bộ vào thư mục `public_html`.
...
3. **Trỏ domain riêng:**
   ...
   - Bật **Enforce HTTPS**.
4. Sau khi có domain, thay `acchoanglong.com` ở `index.html` / `sitemap.xml` / `robots.txt` (xem mục 2).
